...
export const providerSeed = async () => {
    try {
        ...
        const existingProviders = await ProviderDB.findAll({ 
            attributes: ['name'] 
        }); 
        
        const existingNames = existingProviders.map(provider => (provider as any).name);

        // 2. Filtrar el arreglo, manteniendo solo los proveedores cuyos nombres NO existan
        const uniqueProvidersToCreate = providersToCreate.filter(provider => 
            !existingNames.includes(provider.name)
        );

        // 3. Aplicar las fechas a los proveedores que serán insertados
        const finalProviders = uniqueProvidersToCreate.map(provider => ({
            ...provider,
            createdAt: new Date(),
            updatedAt: new Date(),
        }));
        ...
        if (finalProviders.length > 0) {
            ...
            const createdProviders = await ProviderDB.bulkCreate(finalProviders);
