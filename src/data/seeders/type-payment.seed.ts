...
export const typePaymentSeed = async () => {
    try {
        ...
        const existingTypes = await TypePaymentDB.findAll({ 
            attributes: ['name'] 
        }); 
        
        const existingNames = existingTypes.map(type => (type as any).name);

        // 2. Filtrar el arreglo, manteniendo solo los tipos que NO existan
        const uniqueTypesToCreate = typesToCreate.filter(type => 
            !existingNames.includes(type.name)
        );

        // 3. Aplicar las fechas a los tipos que serán insertados
        const finalTypes = uniqueTypesToCreate.map(type => ({
            ...type,
            createdAt: new Date(),
            updatedAt: new Date(),
        }));
        ...
        if (finalTypes.length > 0) {
            ...
            const createdTypes = await TypePaymentDB.bulkCreate(finalTypes);
